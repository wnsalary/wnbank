...
public class GYDXQuartzJob implements WLTJobIFC {
	...
	public void gradeScore(int num) {
		try {
			//为整个考核计划生成计划ID
			String maxId = dmo.getStringValueByDS(null, "select max(ID) from WN_GYDX_TABLE");
	...
	public  void gradeEnd(int num){//定性考核计划结束
		try {
		  String countStr = dmo.getStringValueByDS(null, "select count(*) from WN_GYDX_TABLE where state='评分中' or  fhresult is null ");
		  if( Integer.parseInt(countStr)<=0){
			  return;
		  }else {
			  String planId = dmo.getStringValueByDS(null, "select id from WN_GYDX_TABLE where state='评分中' or fhresult  IS null");
			  WnSalaryServiceIfc service=new WnSalaryServiceImpl();
