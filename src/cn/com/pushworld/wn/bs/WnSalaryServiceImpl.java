...
public class WnSalaryServiceImpl implements WnSalaryServiceIfc {
	...
	public String[][] getTowWeiDate() {
		String[][] date = new String[][] {
				{ "职业形象", "着装", "5.0", "", "按要求着装，形象统一、规范、整洁" },
				{ "职业形象", "三姿", "5.0", "", "站姿、坐姿、走姿端庄大方" },
				{ "营业准备", "及时", "3.0", "", "准备工作要求营业前完成" },
				{
						"物件摆放归位整齐",
						"抽屉",
						"3.0",
						...
						"除各种机具外，物品摆放必须做到规范、有序，不得随意放置，各种凭证、印章、印泥、印鉴册、剪刀、胶水、报表、信贷档案、现金尾箱、登记薄、私人物品等不得放置于桌面上；办理业务需要使用的，使用完毕后需放置于柜面下或抽屉中；未定位摆放、杂乱等不得分" },
				{
						...
						"",
						"除各种机具外，物品摆放必须做到规范、有序，不得随意放置，各种凭证、印章、印泥、印鉴册、剪刀、胶水、报表、信贷档案、现金尾箱、登记薄、私人物品等不得放置于桌面上；办理业务需要使用的，使用完毕后需放置于柜面下或抽屉中；未定位摆放、杂乱等不得分" },
				{ "卫生保洁", "点钞机", "3.0", "", "有积灰、污渍、纸屑等不得分" },
				{ "卫生保洁", "计算机", "3.0", "", "有积灰、污渍、纸屑等不得分" },
				{ "卫生保洁", "地面", "3.0", "", "有积灰、污渍、纸屑等不得分" },
				{ "卫生保洁", "窗面", "3.0", "", "有积灰、污渍、纸屑等不得分" },
				{ "卫生保洁", "柜面", "3.0", "", "有积灰、污渍、纸屑等不得分" },
				{ "声音", "音量", "2.0", "", "适中" },
				{ "声音", "语速", "2.0", "", "清晰适中" },
				{ "表情", "眼神", "2.0", "", "交流、对视" },
				{ "表情", "微笑", "2.0", "", "适时微笑" },
				{ "问候迎接", "呼叫", "3.0", "", "举手示意" },
				{ "问候迎接", "问候", "3.0", "", "首问“您好”/日常称谓，请客入座" },
				{ "接待办理", "义务", "5.0", "",
						"做到先外后内，办理业务执行一次性回复；对非受理范围内业务主动主动告知、引导其到相关窗口办理。" },
				{ "接待办理", "语言", "10.0", "",
						"做到热心、诚心、耐心，自觉使用“请、您好、谢谢、对不起、再见、请问您需要办理什么业务、请稍等、请您查看计数器、请您核对后在这里签字”等文明用语" },
				{ "接待办理", "动作", "5.0", "", "双手递接、掌心朝上" },
				{ "接待办理", "动作", "5.0", "",
						"适时主动与客户交流我社销售产品内容，介绍我社业务，创造销售机会，或请客户协助完成服务满意度评价。" },
				{ "送别结束", "语言", "3.0", "", "请问您还需要办理其他业务吗？/请慢走，再见" },
				{ "其他情况", "1.办业务期间不大声闲谈", "1.0", "", "这些情况在发生时不得分" },
				{ "其他情况", "2.指引动作手掌朝上", "1.0", "", "这些情况在发生时不得分" },
				{ "其他情况", "3.间断业务时未向顾客做请示或表歉意", "1.0", "", "这些情况在发生时不得分" },
				{ "其他情况", "4.手机未设震动", "1.0", "", "这些情况在发生时不得分" },
				{ "其他情况", "5.暂离岗位时椅子未归位", "1.0", "", "这些情况在发生时不得分" },
				{ "其他情况", "6.不必要的肢体动作过多", "1.0", "", "这些情况在发生时不得分" },
				{ "其他情况", "7.暂停业务时未放置示意牌", "1.0", "", "这些情况在发生时不得分" },
				{ "其他情况", "8.长时间业务未主动安抚客户情绪", "1.0", "", "这些情况在发生时不得分" },
				{ "其他情况", "9.有其他被投诉情况", "5.0", "", "这些情况在发生时不得分" },
				{ "营业结束", "卫生保洁", "5.0", "", "这些情况在发生时不得分" },
				{ "总分", "", "", "", "" } };
		return date;
	}
	...
	public String getBMsql(String planid) {
		...
		try {
			...
			HashVO[] vos = dmo.getHashVoArrayByDS(null,
					"select * from sal_target_list where type='部门定性指标'");
	...
	public String getQnedRate(String date_time) {
		try {
			HashMap<String, String> rwMap = dmo.getHashMapBySQLByDS(null,
					"select A,sum(P) from EXCEL_TAB_53 where year||'年'||month||'月'='"
		...
		try {
			...
			HashMap<String, String> lastMonthData = dmo
					.getHashMapBySQLByDS(
							null,
							"SELECT a.mname mname, count(a.CARDNO) CARDNO   FROM (SELECT DISTINCT(CARDNO) CARDNO,MNAME FROM wn_qnedtd WHERE  date_time  LIKE '"
									+ lastMonth + "%') a GROUP BY  a.mname");// 上一个月所做的绩效
			...
			// if (rwMap.size() <= 0) {
			// return "当前客户经理【" + username + "】在当前月【" + date +
			// "】缺少业务数据,请更换日期进行查询";
			// }
			// String deleteSQL =
	...
	public String getTyxwRate(String date_time) {
		...
		try {
			...
			List<String> wdList = Arrays.asList(new String[] { "西海信用社",
					"人民南路分社", "人民中路分社", "营业部", "草海信用社", "龙凤分社", "鸭子塘信用社",
					"新区信用社" });
			List<String> sqlList = new ArrayList<String>();
			HashMap<String, String> userMap = dmo
					.getHashMapBySQLByDS(
			...
			for (String key : keys) {
				...
				System.out.println("key=" + key + ",小微="
						+ getXwNum(local, key, date_time) + ",特约="
			...
			HashMap<String, String> jlMap = dmo
					.getHashMapBySQLByDS(
							...
							"SELECT a.name,a.num num FROM (select bbbb.b name,count(bbbb.b)  num  from (select aaa.c as c,aaa.n as n from (select aa.c,count(aa.c) as n from (select b.b as b ,b.c as c,b.e as e from (select * from excel_tab_10 where year||'-'||month='"
									...
									+ date_time
									+ "' and d='是' or d is null) b on b.c=a.c  where b.b is not null) aa left join (select a,(b+c) as num  from excel_tab_14 where year||'-'||month='"
									+ date_time
									+ "') bb on aa.b=bb.a where bb.num>=aa.e group by aa.c ) aaa left join (select c,count(c) as n from excel_tab_11 where year||'-'||month='"
									+ date_time
									+ "'  and d='是' or d is null group by c ) bbb on bbb.c=aaa.c where aaa.n>=bbb.n) aaaa left join (select b,c from excel_tab_10 where  year||'-'||month='"
									+ date_time
									+ "')  bbbb on aaaa.c=bbbb.c  WHERE bbbb.b NOT  IN ('何超','施宗华','陈举','余崇玺','李顺苍','朱锡刚','罗尧')   group by bbbb.b) a ");
	...
	public String getsjRate(String date_time) {
		...
		try {
			...
			// if (rwMap.size() <= 0) {
			// return "当前客户经理【" + username + "】在当前月【" + date_time +
			// "】缺少业务数据,请更换日期进行查询";
			// }
			...
			String count = (dmo
					.getStringValueByDS(
							...
							"select count(bb.mcht_id) num from (select a.a a ,b.b b from (select * from excel_tab_13 where b='"
									+ local
									+ "' and c='小微商户' and year||'-'||month='"
									+ date_time
									+ "') a left join (select  * from excel_tab_16 where year||'-'||month='"
									+ date_time
									+ "') b on a.d=b.a)  aa left join  (select a.mcht_id mcht_id,a.num num from (select mcht_id, sum(kbsxj_m+sbsxj_m+zqbsxj_m+zzbsxj_m) num from wnbank.t_dis_ifsp_intgr_busi_dtl_cxb  where mcht_prop='小微商户' and  to_char(to_date(day_id,'yyyy-mm-dd'),'yyyy-mm-dd')>='"
									+ date_time
									+ "-01"
		...
		try {
			...
			String count = dmo
					.getStringValueByDS(
							...
							"select count(bb.mcht_id) num from (select a.a a ,b.b b from (select * from excel_tab_13 where b='"
									+ local
									+ "' and c='特约商户' and year||'-'||month='"
									+ date_time
									+ "') a left join (select  * from excel_tab_16 where year||'-'||month='"
									+ date_time
									+ "') b on a.d=b.a)  aa left join  (select a.mcht_id mcht_id,a.num num from (select mcht_id, sum(kbsxj_m+sbsxj_m+zqbsxj_m+zzbsxj_m) num from wnbank.t_dis_ifsp_intgr_busi_dtl_cxb  where mcht_prop='特约商户' and  to_char(to_date(day_id,'yyyy-mm-dd'),'yyyy-mm-dd')>='"
									+ date_time
									+ "-01"
									+ "' and  to_char(to_date(day_id,'yyyy-mm-dd'),'yyyy-mm-dd')<='"
									+ monthEnd
