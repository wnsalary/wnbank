...
//	}
	...
	public String znCount(String curSelectMonthStart, String curSelectDate,
		...
		try {
			...
			// 执行计算结果，并将计算结果导入数据库中
			String sql = "select xx.a mer_id  ,case when  znqk.num  is null then 0 else znqk.num end  znqk ,case when znzz.num is null then 0 else znzz.num end znzz ,case when  znsb.num is null then 0 else znsb.num end znsb ,case when znyl.num is null then 0 else znyl.num end znyl ,case when zndf.num is null then 0 else zndf.num end  zndf, case when dxqy.num is null then 0 else dxqy.num end  dxqy,(case when  znqk.num  is null then 0 else znqk.num end +case when znzz.num is null then 0 else znzz.num end +case when  znsb.num is null then 0 else znsb.num end +case when znyl.num is null then 0 else znyl.num end +case when zndf.num is null then 0 else zndf.num end+case when dxqy.num is null then 0 else dxqy.num end) total   from  ( select a,b,c from  wnsalarydb.excel_tab_83 ) xx left join (select mer_id,sum (num) num from (select mer_id,case when count(mer_id)>=5 then 5 else count(mer_id) end num  from  wnbank.t_dis_ifsp_intgr_txn_dtl where to_char(to_date(biz_dt,'yyyy-mm-dd'),'yyyy-mm-dd')>='"
					+ curSelectMonthStart
