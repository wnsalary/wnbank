...
public class WnUtils {
    /**
     * 根据传入的日期获取到月末日期
     * @param date(日期格式为:20190101或者2019-01-01或者2019年1月1日)
     * @return
     */
	...
	public static String getMonthEnd(String date){
		...
		try {
			...
			int  year=Integer.parseInt(new SimpleDateFormat("yyyy").format(parse));
			int  month=Integer.parseInt(new SimpleDateFormat("MM").format(parse));
			String day="";
			switch (month) {
			case 1:
			case 3:
		...
		result=year+"-"+(month>=10?String.valueOf(month):String.valueOf("0"+month))+"-"+day;
		} catch (Exception e) {
			e.printStackTrace();
		}
		return result;
	...
	public  static  String getYearEnd(String date){
		...
		try {
			...
			Date parse = simple.parse(date);
			int  year=Integer.parseInt(new SimpleDateFormat("yyyy").format(parse))-1;
			result=year+"-12-31";
		} catch (Exception e) {
			e.printStackTrace();
		}
