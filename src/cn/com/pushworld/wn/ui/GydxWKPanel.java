...
//	private CommDMO dmo = new CommDMO();
	...
	public void initialize() {
		try {
			USERCODES = UIUtil.getHashMapBySQLByDS(null, "select id,code from PUB_CORP_DEPT");
			USERTYPE = UIUtil.getHashMapBySQLByDS(null, "select USERCODE,POSTNAME from  V_PUB_USER_POST_1 ");//对于人员评分表，不同的人看到柜员评分表中的内容
			KJMAP = UIUtil.getHashMapBySQLByDS(null, "SELECT DEPTCODE,USERCODE FROM V_PUB_USER_POST_1 WHERE POSTNAME='委派会计'");
			ZRMAP=UIUtil.getHashMapBySQLByDS(null, "SELECT DEPTCODE,USERCODE FROM V_PUB_USER_POST_1 WHERE POSTNAME like '%主任%' ORDER BY POSTNAME DESC");
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		if ("主任".equals(USERTYPE.get(PFSUERCODE).toString()) || "副主任".equals(USERTYPE.get(PFSUERCODE).toString())) {
	...
	private void saveScore() {//点击保存评分，这个时候只计算柜员的最终得分，不修改状态。保存功能可以由委派会计和网点主任使用
		try {
			...
			String deptcode=billListPanel_User_Post.getSelectedBillVO().getStringValue("deptcode");
			...
			UpdateSQLBuilder update = new UpdateSQLBuilder(billListPanel_User_check.getTempletVO().getTablename());//设置修改
			for (int i = 0; i < billVOs.length; i++) {
				if ("总分".equals(billVOs[i].getStringValue("xiangmu"))) {
					continue;
				}
				fenzhi=Double.parseDouble(billVOs[i].getStringValue("fenzhi"));//获取到当前考核项的分值
				fhreason=billVOs[i].getStringValue("FHREASON");//获取到复核理由
				if (billVOs[i].getStringValue("KOUFEN") != null && !billVOs[i].getStringValue("KOUFEN").isEmpty()) {
					koufen = Double.parseDouble(billVOs[i].getStringValue("KOUFEN"));
					if (fenzhi < koufen) {
						sb.append("第" + (i+1) + "行数据项目为[" + billVOs[i].getStringValue("khsm") + "],项目为[" + billVOs[i].getStringValue("xiangmu") + "]扣分项大于分值  \n");
					}
				} else {
					sb.append("第" + (i+1) + "行数据项目为[" + billVOs[i].getStringValue("khsm") + "],项目为[" + billVOs[i].getStringValue("xiangmu") + "]扣分项为空  \n");
				}
				result = result + koufen;
				update.setWhereCondition("usercode='"+billVOs[i].getStringValue("usercode")+"' and khsm='"+billVOs[i].getStringValue("khsm")+"' and state='评分中'");
				update.putFieldValue("KOUFEN", koufen);
				update.putFieldValue("FHREASON", fhreason);
				list.add(update.getSQL());
			}
			if(sb.length()<=0){//
//				billListPanel_User_check.setRealValueAt(String.valueOf(100 - result), billVOs.length - 1, "fenzhi");
				update.setWhereCondition("usercode='"+vov.getStringValue("usercode")+"' and state='评分中' and xiangmu='总分'");
				...
				update2.setWhereCondition("USERCODE='" + gyusercode + "' and state='评分中'");
				update2.putFieldValue("PFUSERNAME", PFUSERNAME);
				update2.putFieldValue("PFUSERCODE", PFSUERCODE);
				update2.putFieldValue("pftime", new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(new Date()));//插入评分时间
...
//				MessageBox.show(this,update2.getSQL());
	...
	private void endScore() {//结束评分 1.计算最后总的分值；2.修改评分状态
		try {
			...
			String[] state = UIUtil.getStringArrayFirstColByDS(null, "select state from wn_gydx_table where state='评分中' AND USERCODE='" + gyUserCode + "'");
			if(state==null || state.length<=0){
				MessageBox.show(this, "当前柜员【" + gyUserName + "】定性指标考核已经结束，无须重复结束！");
				return;
			}
//			HashVO[] vos = UIUtil.getHashVoArrayByDS(null, "select USERDEPT,USERNAME,PFTIME from wn_gydx_table where 1=1 and KOUFEN is null group by USERDEPT,USERNAME,PFTIME order by USERDEPT");
			BillVO[] bos = billListPanel_User_check.getBillVOs();
			StringBuffer sb = new StringBuffer();//获取到当前评分项
			HashMap<String, String> map = UIUtil.getHashMapBySQLByDS(null, "select id,code from pub_corp_dept");
			UpdateSQLBuilder update3=new UpdateSQLBuilder(billListPanel_User_check.getTempletVO().getTablename());
			update3.setWhereCondition("USERCODE='"+gyUserCode+"' and state='评分中'");
			for (int j = 0; j < bos.length; j++) {
				if("总分".equals(bos[j].getStringValue("xiangmu"))){
					continue;
				}
				if(bos[j].getStringValue("KOUFEN")==null||bos[j].getStringValue("KOUFEN").isEmpty()){
					sb.append("柜员名为[" + bos[j].getStringValue("USERNAME") + "],考核项为["+bos[j].getStringValue("xiangmu")+"]评分未完成！ \n");
				}
			}
			...
			if(sb.length()> 0){//存在未评分的项
				if (MessageBox.confirm(this, "当前柜员【"+gyUserName+"】尚未完成,确定强制结束吗？  \n" + sb.toString())) {
					...
					for(int j = 0; j < bos.length; j++){
						...
						update.setWhereCondition(" USERCODE='"+gyUserCode+"'  and khsm='"+bos[j].getStringValue("khsm")+"' and  state='评分中'");
						fenzhi=Double.parseDouble(bos[j].getStringValue("fenzhi"));
						if(bos[j].getStringValue("KOUFEN")==null){
							koufen=0.0;
						}
						koufen=Double.parseDouble(bos[j].getStringValue("KOUFEN"));
						if (fenzhi <= koufen) {
						    koufen=fenzhi;
						}
						result=result-koufen;
					...
					update2.setWhereCondition("USERCODE='" + gyUserCode + "' and state='评分中' and xiangmu='总分'");
...
//					billListPanel_User_check.refreshData();
				...
				MessageBox.show(this, "评分结束成功");
				pfTime=UIUtil.getStringValueByDS(null,"SELECT max(PFTIME) FROM wn_gydx_table WHERE USERCODE='"+gyUserCode+"'");
				String sql="select * from wn_gydx_table where usercode='"+gyUserCode+"' and pftime='"+pfTime+"' ";
				billListPanel_User_check.QueryData(sql);
//				billListPanel_User_check.refreshData();
			}
		} catch (Exception e) {
	...
	public void onBillListSelectChanged(BillListSelectionEvent e) {
		try {
			if (e.getSource() == billListPanel_User_Post) {
				...
				pfTime=UIUtil.getStringValueByDS(null, "select max(pftime) from  WN_GYDX_TABLE where usercode='"+vo.getStringValue("usercode")+"'");
				String sql = "select * from wn_gydx_table where 1=1 and usercode='" + vo.getStringValue("usercode") + "' and (pftime='"+pfTime+"')  order by xiangmu";
				billListPanel_User_check.QueryData(sql);
			}
