...
public class GydxdfWKPanel extends AbstractWorkPanel implements ActionListener {
	...
	public void initialize() {
		...
		listPanel.addBatchBillListButton(new WLTButton[] { btn_delete,btn_ks, btn_end });
	...
	private void gradeScore() {//开始考核,为每个柜员生成定性考核计划
		try{
			...
			if(!"未评分".equals(state)){//获取到当前状态
				MessageBox.show(this,"当前柜员定性考核计划【"+selectedBillVO.getStringValue("planname")+"】状态为【"+state+"】,无法再次生成！！！");
			    return;
			}
			//在生成考核计划之前，判断是否存在其他考核计划状态是否结束
			String  countString=UIUtil.getStringValueByDS(null, "select count(*) from WN_GYDXPLAN where id!='"+id+"' and state='评分中'");
			Integer count=Integer.parseInt(countString);
			if(count!=0){
				HashVO[] plans = UIUtil.getHashVoArrayByDS(null, "select * from WN_GYDXPLAN where state='评分中'");
			    StringBuffer  buffer=new StringBuffer("");
				for (int i = 0; i < plans.length; i++) {
					buffer.append(plans[i].getStringValue("planname"));
				}
				MessageBox.show(this,"存在柜员定性考核计划【"+buffer+"】评分尚未结束，请先结束其他计划");
				return;
			}
			//判断当前考核计划是否全部评分完成以及复核完成
			String countString2=UIUtil.getStringValueByDS(null, "SELECT COUNT(*) FROM wn_gydx_table WHERE state='评分中' OR fhresult IS NULL");
...
//			if(count2>0){
//				int  sumcount=MessageBox.showOptionDialog(this, "当前考核计划中存在是否评分尚未结束或尚未复核的柜员，是否结束当前考核计划","提示", new String[] { "是", "否" },1);
//				if(sumcount!=0){
...
//		    
	...
	private void gradeEnd() {//结束考核
		try {
			...
			if(!"评分中".equals(state)){
				MessageBox.show(this,"当前考核计划【"+planName+"】状态为【"+state+"】,无法结束。");
				return;
			}
			//当用户选择确定结束时,记得让用户确定一下
			String count = UIUtil.getStringValueByDS(null, "select count(*) from wn_gydx_table where state='评分中' or fhresult='复核未通过'");
			...
			String sql="update  WN_GYDXPLAN  set STATE='评分结束' where id='"+planid+"'";
	...
	public void gradeScore(int num) {
		try {
			//为整个考核计划生成计划ID
			String maxId = UIUtil.getStringValueByDS(null, "select max(ID) from WN_GYDX_TABLE");
	...
	public  void gradeEnd(int num){//定性考核计划结束
		try {
		  String countStr = UIUtil.getStringValueByDS(null, "select count(*) from WN_GYDX_TABLE where state='评分中' or  fhresult is null ");
		  if( Integer.parseInt(countStr)<=0){
			  return;
		  }else {
			  String planId = UIUtil.getStringValueByDS(null, "select id from WN_GYDX_TABLE where state='评分中' or fhresult  IS null");
			  WnSalaryServiceIfc service=new WnSalaryServiceImpl();
