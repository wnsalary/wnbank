...
public class ManagerDxPlanWKPanel extends  AbstractWorkPanel implements ActionListener {
...
//		try {
//			hashVos=UIUtil.getHashVoArrayByDS(null, "select * from sal_person_check_list where 1=1  and (type='61')  and (1=1)  order by seq");
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
	    ...
	    listPanel.addBatchBillListButton(new WLTButton[]{btn_delete,btn_start,btn_end});
	...
	private void endGradeScore() {
		...
		//判断当前评分是否结束
		String count = UIUtil.getStringValueByDS(null, "select count(*) num from WN_MANAGERDX_TABLE where state='评分中' and PFUSERNAME is null");
		...
		UIUtil.executeUpdateByDS(null, "update wn_managerplan_table set state='评分结束' where id='"+id+"'");
	...
	private void startGradeScore() {//为每个客户经理生成考核计划
		try {
			...
			if(!"未评分".equals(state)){
				MessageBox.show(this,"当前客户经理定性考核计划【"+selectedBillVO.getStringValue("planname")+"】状态为【"+state+"】,无法再次生成！！！");
			    return;
			}
			String  countString=UIUtil.getStringValueByDS(null, "select count(*) from wn_managerplan_table where id!='"+id+"' and state='评分中'");
			Integer count=Integer.parseInt(countString);
			if(count>0){
				HashVO[] plans = UIUtil.getHashVoArrayByDS(null, "select * from wn_managerplan_table where state='评分中'");
			    StringBuilder buffer=new StringBuilder();
			    for (int i = 0; i < plans.length; i++) {
			    	buffer.append(plans[i].getStringValue("planname"));
				}
			    MessageBox.show(this,"存在客户经理定性考核计划【"+buffer+"】评分尚未结束，请先结束其他计划");
			    return;
			}
			final WnSalaryServiceIfc service = (WnSalaryServiceIfc) UIUtil.lookUpRemoteService(WnSalaryServiceIfc.class);
			new SplashWindow(this,new AbstractAction() {
			...
			UIUtil.executeUpdateByDS(null, "update wn_managerplan_table set state='评分中' where id='"+id+"'");
		    listPanel.refreshData();
		} catch (Exception e) {
			e.printStackTrace();
		}
