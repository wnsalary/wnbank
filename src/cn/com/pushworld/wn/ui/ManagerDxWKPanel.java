...
public class ManagerDxWKPanel extends AbstractWorkPanel implements BillTreeSelectListener, BillListSelectListener, ActionListener {
	...
	public void initialize() {
		...
		billListPanel_User_check = new BillListPanel("WN_MANAGERDX_TABLE_Q01_ZPY");//客户经理定性评分表
		...
		billListPanel_User_Post.queryDataByCondition("deptcode='" + PFDEPTCODE + "' and POSTNAME like '%客户经理%'", "seq,usercode");
		this.add(splitPanel);
	}
		...
		try {
			...
			for (int i = 0; i < billVOs.length; i++) {
				...
				if (billVOs[i].getStringValue("koufen") == null || billVOs[i].getStringValue("koufen").isEmpty()) {
					str.append("当前考核客户经理【" + billVOs[i].getStringValue("username") + "】中考核项为【" + billVOs[i].getStringValue("xiangmu") + "】扣分项为空,请修正");
				} else {
					fenzhi = Double.parseDouble(billVOs[i].getStringValue("fenzhi"));
					koufen = Double.parseDouble(billVOs[i].getStringValue("koufen"));
					if (fenzhi < koufen) {
						str.append("当前考核客户经理【" + billVOs[i].getStringValue("username") + "】中考核项为【" + billVOs[i].getStringValue("xiangmu") + "】扣分超过此项分值,请修正");
					}
				}
			}
			...
			for (int i = 0; i < billVOs.length; i++) {
				...
				update.putFieldValue("pfuserdept", USERCODES.get(PFUSERDEPT).toString());
				update.putFieldValue("pftime", new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(new Date()));
				_sqllist.add(update.getSQL());
			}
			UpdateSQLBuilder update2 = new UpdateSQLBuilder("wn_managerdx_table");
			update2.setWhereCondition("usercode='" + usercode + "' and khsm='总分' and state='评分中'");
			...
			update2.putFieldValue("pfuserdept", USERCODES.get(PFUSERDEPT).toString());
			update2.putFieldValue("pftime", new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(new Date()));
			_sqllist.add(update2.getSQL());
			UIUtil.executeBatchByDS(null, _sqllist);
			String sql = "select * from wn_managerdx_table where usercode='" + usercode + "' and state='评分中'";
	...
	private void endScore() {
		try {
			...
			String usercode = selectedBillVO.getStringValue("usercode");
			//判断当前客户经理是否已经结束评分
			String[] state = UIUtil.getStringArrayFirstColByDS(null, "select state from wn_managerdx_table where state='评分中' AND USERCODE='" + usercode + "'");
			if (state == null || state.length <= 0) {
				MessageBox.show(this, "当前客户经理【" + usercode + "】定性指标考核已经结束，无须重复结束！");
				return;
			}
			...
			for (int j = 0; j < bos.length; j++) {
				...
				if (bos[j].getStringValue("KOUFEN") == null || bos[j].getStringValue("KOUFEN").isEmpty()) {
					sb.append("客户经理名为[" + bos[j].getStringValue("USERNAME") + "],考核项为[" + bos[j].getStringValue("xiangmu") + "]评分未完成！ \n");
				}
			}
			if (sb.length() > 0) {
				if (MessageBox.confirm(this, "当前柜员【" + usercode + "】尚未完成,确定强制结束吗？  \n" + sb.toString())) {
					double fenzhi = 0.0;
					double koufen = 0.0;
					double result = 0.0;
					...
					for (int i = 0; i < bos.length; i++) {//保存每一项的明细
						...
						koufen = Double.parseDouble(bos[i].getStringValue("koufen"));
						fenzhi = Double.parseDouble(bos[i].getStringValue("fenzhi"));
						if (fenzhi < koufen) {
							koufen = fenzhi;
						}
						result = result + koufen;
						update.setWhereCondition("usercode='" + usercode + "' and state='评分中' and khsm='" + bos[i].getStringValue("khsm") + "'");
					...
					update2.setWhereCondition("usercode='" + usercode + "' and state='评分中' and khsm='总分'");
					update2.putFieldValue("pfusername", PFUSERNAME);
					update2.putFieldValue("pfusercode", PFSUERCODE);
					update2.putFieldValue("pfuserdept", USERCODES.get(PFUSERDEPT).toString());
					update2.putFieldValue("pftime", new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(new Date()));
			...
			String pftime = UIUtil.getStringValueByDS(null, "select max(pftime) from wn_managerdx_table where usercode='" + usercode + "'");
			billListPanel_User_check.QueryData("select * from wn_managerdx_table where usercode='" + usercode + "' and pftime='" + pftime + "'");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void onBillListSelectChanged(BillListSelectionEvent e) {
		try {
			if (e.getSource() == billListPanel_User_Post) {
				...
				String pfTime = UIUtil.getStringValueByDS(null, "select max(pftime) from wn_managerdx_table where usercode='" + vo.getStringValue("usercode") + "'");
				String sql = "select * from wn_managerdx_table where 1=1 and  usercode='" + vo.getStringValue("usercode") + "' and  pftime='" + pfTime + "' order by xiangmu";
				billListPanel_User_check.QueryData(sql);
