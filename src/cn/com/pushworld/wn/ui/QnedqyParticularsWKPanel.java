 ...
 *黔农E贷签约户数查询
 */
public class QnedqyParticularsWKPanel extends AbstractWorkPanel implements BillListHtmlHrefListener,ActionListener {
...
//         listPanel.addBillListHtmlHrefListener(this);
	...
	public void onBillListHtmlHrefClicked(BillListHtmlHrefEvent e) {
		if(e.getSource()==listPanel){
			...
			dialog.getBilllistPanel().QueryDataByCondition("username='"+vo.getStringValue("E")+"' and date_time='"+vo.getStringValue("date_time")+"'");
...
//			dialog.setBtn_confirmVisible(false);
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==list.getQuickQueryPanel()){//获取到快速查询事件
			try {
//				String username=ClientEnvironment.getInstance().getLoginUserName();//获取到登录人的姓名
//				//获取到当前查询条件
//				String condition = listPanel.getQuickQueryPanel().getQuerySQLCondition();//获取到当前查询条件
//				String sql="select * from V_qnedqy_zpy where 1=1 and E='"+username+"' ";
