...
public class SjyhkhParticularsWKPanel extends AbstractWorkPanel implements ActionListener,BillListHtmlHrefListener {
...
//		listPanel.addBillListHtmlHrefListener(this);
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==list.getQuickQueryPanel()){//
			String username=ClientEnvironment.getInstance().getLoginUserName();//获取到当前登录人姓名
			try {
//				String condition=listPanel.getQuickQueryPanel().getQuerySQLCondition();//获取到查询条件
//				if(condition.contains("date_time")){//如果存在日期，则对日期进行处理
//					System.out.println("当前查询条件为:"+condition);
//					condition = condition.substring(condition.indexOf("=") + 2, condition.lastIndexOf("and") - 2);
//					condition=condition.substring(0,condition.lastIndexOf("-")) ;
//					String sql="select * from V_sjyh where xd_col2='"+username+"' and  date_time='"+condition+"'";
...
//				}
	...
	public void onBillListHtmlHrefClicked(BillListHtmlHrefEvent e) {
		if(e.getSource()==listPanel){
			...
			dialog.getBilllistPanel().QueryDataByCondition("username='"+username+"' and date_time='"+date_time+"'");
