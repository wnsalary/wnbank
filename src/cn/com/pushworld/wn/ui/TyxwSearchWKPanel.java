	...
	private void QuerySearch() {
		try {
		  ...
		  if(date_time!=null&&!date_time.isEmpty()){
			  ...
			  sql="SELECT  distinct(a.mcht_id) mcht_id,b.mcht_name,b.mcht_prop,b.client_local,a.num,a.cus_name,b.date_time,CASE WHEN b.mcht_prop='小微商户' THEN (CASE WHEN a.num>=5 THEN '合格' ELSE '不合格' END ) ELSE (CASE WHEN a.num>=10 THEN '合格' ELSE '不合格' end) end db FROM (SELECT mcht_id,cus_name,sum(num) num  FROM v_wn_tyxw WHERE date_time='"+date_time+"' GROUP  BY mcht_id,cus_name) a LEFT JOIN (SELECT * FROM v_wn_tyxw WHERE date_time='"+date_time+"') b ON a.mcht_id=b.mcht_id WHERE b.mcht_name IS NOT NULL";
		  }else{
			  MessageBox.show(this,"请输入查询日期");
			  return;
		  }
		  String mcht_id=listPanel.getQuickQueryPanel().getCompentRealValue("MCHT_ID");//商户号
		  if(mcht_id!=null&&!mcht_id.isEmpty()){
			  sql=sql+" and a.mcht_id like '%"+mcht_id+"%' ";
		  }
		  
		  String mcht_prop=listPanel.getQuickQueryPanel().getCompentRealValue("MCHT_PROP");//商户类型
		  if(mcht_prop!=null && !mcht_prop.isEmpty()){
			  sql=sql+" and b.mcht_prop like '%"+mcht_prop+"%' ";
		  }
//		  
		  String client_local=listPanel.getQuickQueryPanel().getCompentRealValue("CLIENT_LOCAL");
		  if(client_local!=null && !client_local.isEmpty()){
			  sql=sql+" and b.client_local like '%"+client_local+"%' ";
		  }
		  
		  String cus_name=listPanel.getQuickQueryPanel().getCompentRealValue("CUS_NAME");//客户经理
		  if(cus_name!=null && !cus_name.isEmpty()){
			  sql=sql+" and a.CUS_NAME like '%"+cus_name+"%'";
		  }
		  String db=listPanel.getQuickQueryPanel().getCompentRealValue("DB");
